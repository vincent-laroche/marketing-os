...
Took over the audit handoff. The handoff targets a Manus sandbox (`/home/ubuntu/hairsolutions-campaign-os`,
`/home/ubuntu/github-marketing-os`, `/mnt/…`) that **does not exist on this machine**, so the Marketing OS
application could not be reached, changed, validated or checkpointed. The application half of #118 and all
of #119 remain blocked on environment access — recorded on #117 and #119.

Delivered from this repository instead, in #124:

- **#118 authoritative half.** `github-campaign-os/module-availability.json`, generated by
  `tools/github_campaign_os/module_availability.py` and wired into the existing
  `build_manifest --write/--check` reproducibility contract. All 36 canonical emails carrying
  `Missing Modules` resolve to approved module trios; 89 requirements as 74 exact declared labels,
  11 documented aliases, 4 punctuation folds. #118's stated normalisation was incomplete —
  `Grid - Collections 4` is declared `Grid - Collections (4)`, and slug matching alone would have left
  NL-03, NL-08, NL-13 and NL-18 falsely blocked.
- **#125.** `validate_repository.py` required `pull-requests: write` on the publication ledger job;
  commit 3763e53 deliberately dropped it when publication moved to a manual PR handoff. Expectation
  corrected rather than the privilege re-granted.
- **#126.** `manifest.json` under-reported 14 ledger-backed preview URLs (stale in the safe direction).
  Regenerated from a clean worktree at `origin/main`; the accompanying test asserted URLs were
  "currently blank" and now asserts the durable §9 ledger-derivation contract.

Also filed **#127** — `shopify-messaging/PHASE5-PLAN.md` is cited by AGENTS.md §7, README.md and
PROJECT.md as the consent-tag authority but has never existed in this repository. Needs Vincent's source.

Repository suite: 45 passed, 341 subtests (baseline 34 passed / 141 subtests with 2 failures).
Worker `marketing-os-notion-sync`: `npm run check` clean, `npm test` 10/10. Live Worker endpoints,
D1 and the Cloudflare schedule were not re-read — no deployment credentials here.

Still owner-blocked and untouched: #120 (see the proof-bank and consent findings recorded there),
#121, #122, #123.
