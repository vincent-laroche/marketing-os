...
export async function compilePreview(args: PreviewArgs, capturePreview: typeof capture = capture): Promise<void> {
  ...
  const identity = {repository: "vincent-laroche/marketing-os", campaign: selection.campaign_key, email: args.emailCode, source_path: source, source_sha256: sha256(sourceContents), commit: args.commitSha, issue: args.issue, pr: args.pr, issue_url: `https://github.com/vincent-laroche/marketing-os/issues/${args.issue}`, pr_url: `https://github.com/vincent-laroche/marketing-os/pull/${args.pr}`, persona: args.persona, states: args.states, fixture_sha256: sha256(JSON.stringify(fixture)), compiler_version: "1.0.0", compiler_lock_sha256: sha256(lock), generated_at: new Date().toISOString(), visibility: args.visibility, workflow: args.workflowRun ? {run: args.workflowRun!, attempt: args.workflowAttempt!, revision: args.workflowRevision!} : undefined};
