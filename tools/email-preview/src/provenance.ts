...
export function provenance(args: PreviewArgs, source: string, rendered: string, extras: Pick<Provenance, "output_sha256" | "campaign_key" | "fixture_sha256" | "compiler_lock_sha256" | "generated_at" | "visibility"> & {identity: Record<string, unknown>}): Provenance {
  ...
  return {
    ...
    issue_url: `https://github.com/vincent-laroche/marketing-os/issues/${args.issue}`,
    pr_url: `https://github.com/vincent-laroche/marketing-os/pull/${args.pr}`,
...
const provenanceSchema = z.object({
  schema_version: z.literal(1), email_code: z.string().min(1), source_path: z.string().min(1), source_commit_sha: z.string().regex(/^[0-9a-f]{40}$/), related_issue: z.number().int().positive(), related_pr: z.number().int().positive(), persona: z.string().min(1), states: z.array(z.string()).min(1), compiler_version: z.string().min(1), source_sha256: z.string().regex(/^[0-9a-f]{64}$/), rendered_sha256: z.string().regex(/^[0-9a-f]{64}$/), outputs: z.tuple([z.literal("rendered.html"), z.literal("desktop.png"), z.literal("mobile.png")]), output_sha256: z.record(z.string(), z.string().regex(/^[0-9a-f]{64}$/)), repository: z.literal("vincent-laroche/marketing-os"), campaign_key: z.string().regex(/^campaign:/), fixture_sha256: z.string().regex(/^[0-9a-f]{64}$/), compiler_lock_sha256: z.string().regex(/^[0-9a-f]{64}$/), generated_at: z.string().datetime(), visibility: z.enum(["private", "public"]), issue_url: z.string().url(), pr_url: z.string().url(), workflow: z.object({run: z.string(), attempt: z.string(), revision: z.string()}).optional()
}).strict();
