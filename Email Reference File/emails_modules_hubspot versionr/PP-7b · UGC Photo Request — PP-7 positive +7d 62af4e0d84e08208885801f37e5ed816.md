...
Hubspot Matched: Yes
Module Stack: (Header - Centered logo) → (Hero - Text-led) → (Text - Opening) → (Text - Customer snapshot) → (Photo - Feature story: example UGC) → (Testimonial: what a good one looks like) → (Text - Offer discount: incentive) → [List - Questions] → [Text - Reassurance] → (Button - Primary CTA) → [List - Support strip] → (Footer - Social)
